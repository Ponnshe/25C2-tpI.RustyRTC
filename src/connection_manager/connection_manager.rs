...
impl ConnectionManager {
    ...
    pub fn apply_remote_sdp(&mut self, remote: &str) -> Result<OutboundSdp, ConnectionError> {
        ...
        let out = match self.signaling {
            SignalingState::Stable => {
                let (remote_is_ice_lite, _ufrag, _pwd) = self.extract_and_store_remote_ice_meta(&sdp)?;
            ...
            SignalingState::HaveLocalOffer => {
                ...
                let (_remote_is_ice_lite, _ufrag, _pwd) = self.extract_and_store_remote_ice_meta(&sdp)?;
                self.extract_and_store_rtp_meta(&sdp)?;
                self.remote_description = Some(sdp);
                self.signaling = SignalingState::Stable;
                Ok(OutboundSdp::None)
    ...
    pub fn extract_and_store_rtp_meta(&mut self, remote_sdp: &Sdp) -> Result<(), ConnectionError> {
        ...
        for m in remote_sdp.media() {
            if !m.proto().to_uppercase().contains("RTP") { continue; }

            let allowed_pts: HashSet<u8> = m.fmts().iter().filter_map(|fmt| fmt.parse::<u8>().ok()).collect();

            for a in m.attrs() {
                if a.key() != "rtpmap" { continue; }
                let raw = a.value().ok_or_else(|| ConnectionError::RtpMap("Wrong value".into()))?;
                let rm: RtpMap = raw.parse().map_err(|_| ConnectionError::RtpMap("Failed parsing rtpmap".into()))?;
                if !allowed_pts.is_empty() && !allowed_pts.contains(&rm.payload_type) { continue; }

                discovered.push(RtpCodec::with_name(rm.payload_type, rm.clock_rate, rm.encoding_name.clone()));
            }
        }
    ...
    pub fn apply_remote_trickle_candidate(&mut self, cand_attr_line: &str) -> Result<(), ConnectionError> {
        let ice_and_sdp: ICEAndSDP = cand_attr_line.parse().map_err(|_| ConnectionError::IceAgent)?;
    ...
    fn extract_and_store_remote_ice_meta(&mut self, remote: &Sdp) -> Result<(bool, Option<String>, Option<String>), ConnectionError> {
        ...
        for a in remote.attrs() {
            match a.key() {
                ...
                "ice-ufrag" => { ufrag = a.value().map(ToOwned::to_owned); }
                "ice-pwd" => { pwd = a.value().map(ToOwned::to_owned); }
                _ => {}
            }
        }
        ...
        for m in remote.media() {
            for a in m.attrs() {
                match a.key() {
                    ...
                    "ice-ufrag" => if ufrag.is_none() { ufrag = a.value().map(ToOwned::to_owned); }
                    "ice-pwd" => if pwd.is_none() { pwd = a.value().map(ToOwned::to_owned); }
                    "ice-lite" => remote_is_ice_lite = true,
                    _ => {}
                }
            }
    ...
    pub fn start_connectivity_checks(&mut self) -> Result<(), ConnectionError> {
        if self.local_description.is_none() || self.remote_description.is_none() {
            return Err(ConnectionError::Negotiation("SDP not complete for ICE".into()));
        }

        self.ice_phase = IcePhase::Gathering;
        if self.ice_agent.local_candidates.is_empty() {
            self.ice_agent.gather_candidates().map_err(|_| ConnectionError::IceAgent)?;
        }
    ...
    const fn set_ice_role_from_signaling(&mut self, we_are_offerer: bool, remote_is_ice_lite: bool) {
    ...
    pub fn run_ice_reactor_blocking(&mut self, total_ms: u64) {
        ...
        let sockets: Vec<Arc<UdpSocket>> = self.ice_agent.local_candidates.iter()
            .filter_map(|c| c.socket.clone()).collect();

        for sock in &sockets { let _ = sock.set_read_timeout(Some(Duration::from_millis(40))); }
        let mut buf = [0u8; 1500];

        while Instant::now() < deadline && self.ice_agent.nominated_pair.is_none() {
            for sock in &sockets {
                match sock.recv_from(&mut buf) {
                    Ok((n, from)) => { self.ice_agent.handle_incoming_packet(&buf[..n], from); },
    ...
    fn build_media_description(&mut self) -> SDPMedia {
        ...
        let formats = if self.local_codecs.is_empty() {
            ...
            self.local_codecs.iter().map(|c| c.rtp.payload_type.to_string()).collect()
        };
        media_desc.set_fmts(formats);
        media_desc.set_connection(Some(SDPConnection::new(DEFAULT_NET_TYPE, DEFAULT_ADDR_TYPE, DEFAULT_CONN_ADDR)));
        ...
        if self.local_codecs.is_empty() {
            attrs.push(SDPAttribute::new("rtpmap", Some("96 H264/90000".to_owned())));
        } else {
            for descriptor in &self.local_codecs {
                let codec = &descriptor.rtp;
                let name = if codec.name.is_empty() { "H264" } else { &codec.name };
                ...
                if let Some(fmtp) = &descriptor.fmtp {
                    attrs.push(SDPAttribute::new("fmtp", Some(format!("{} {fmtp}", codec.payload_type))));
                }
            }
        }
...
const fn is_probably_offer(_sdp: &Sdp) -> bool { false }
