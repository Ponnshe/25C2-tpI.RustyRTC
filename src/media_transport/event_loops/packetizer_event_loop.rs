use std::{collections::HashMap, sync::{Arc, Mutex, atomic::{AtomicBool, Ordering}, mpsc::{Receiver, RecvTimeoutError, Sender}}, thread::JoinHandle, time::Duration};

use crate::{
    app::log_sink::LogSink, core::{events::EngineEvent, session::Session}, media_transport::{
...
impl PacketizerEventLoop {
    ...
    pub fn stop(&mut self){
        sink_info!(
            self.logger,
            "[MT Event Loop MA] Stopping the event loop"
        );
